...
uniquifyProg (Prog decs) = binds withDecl decs $ pure . Prog

withDecl :: (MonadUniquify m) => UncheckedDecl -> (UniqueDecl -> m a) -> m a
withDecl (Def b) m = withBind b $ m . Def . NE.head
withDecl (Entry f params mt body _ pos) m = do
  ...
  (params', body') <-
    binds (withPatParam uniquifyTypeExp) params $ \params' -> do
      ...
      pure (params', body')
  withParam f $ \f' ->
    m $ Entry f' params' mt' body' NoInfo pos
