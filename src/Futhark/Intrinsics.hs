...
translate name = M.findWithDefault (intrinsic name) name scalarOps

scalarOps :: Map Text Translate
scalarOps =
  M.fromList $
...
intrinsic "head" [Arg xs] _ = do
  fxs <- argVar "xs" xs
  txs <- F.lookupType fxs
  F.letSubExp "head" $ F.BasicOp $ F.Index fxs $ F.fullSlice txs [F.DimFix $ constInt64 0]
intrinsic "tail" [Arg xs] t = do
...
intrinsic "length" [Arg xs] _ =
  F.arraySize 0 <$> compileArrayType (argType xs)
intrinsic "append" [Arg xs, Arg ys] t = do
  rt <- compileArrayType t
  fxs <- argVar "xs" xs
  fys <- argVar "ys" ys
  F.letSubExp "append" $ F.BasicOp $ F.Concat 0 (fxs NE.:| [fys]) (F.arraySize 0 rt)
intrinsic "reverse" [Arg xs] _ =
  F.Var <$> (compileReverse =<< argVar "xs" xs)
intrinsic "reduce" [FunArg op, Arg xs] _ = do
  ...
  compileFold "reduce" op first rest
intrinsic "reduce/zero" [FunArg op, Arg zero, Arg xs] _ =
  compileReduce (F.Reduce F.Noncommutative op [argSExp zero]) xs
...
intrinsic "flatten" [Arg xs] t =
  reshapeTo "flatten" t =<< argVar "xs" xs
intrinsic "reshape" [Arg xs] t =
  reshapeTo "reshape" t =<< argVar "xs" xs
intrinsic "transpose2d" [Arg xs] _ = do
  fxs <- argVar "xs" xs
  F.letSubExp "transpose" $ F.BasicOp $ F.Rearrange fxs [1, 0]
intrinsic "index" [Arg xs, Arg i] _ = do
  fxs <- argVar "xs" xs
  F.letSubExp "index" =<< F.eIndex fxs [F.eSubExp $ argSExp i]
intrinsic "index2d" [Arg xs, Arg is] _ = do
  fxs <- argVar "xs" xs
  fis <- argVar "is" is
...
intrinsic "undefined" [] t = do
  ...
  blank <- F.letSubExp "blank" =<< F.eBlank =<< compileArrayType t
  F.certifying (F.Certs [failure]) $
    F.letSubExp "undefined" $
      F.BasicOp $
        F.Opaque F.OpaqueNil blank
