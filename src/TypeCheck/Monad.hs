...
withTypeParam (AtomTypeParam vname) m =
  localVNames
    ( \vnames ->
        vnames
...
-- | Bind a source-level type parameter expression into a local environment.
withTypeParamExp :: (MonadCheck m) => TypeParamExp VName -> (TypeParamExp VName -> m a) -> m a
withTypeParamExp (TEAtomTypeParam vname) m =
  localVNames
    ( \vnames ->
        vnames
          { vnameEnvAtomTypeVars = S.insert vname $ vnameEnvAtomTypeVars vnames
          }
    )
    $ m (TEAtomTypeParam vname)
withTypeParamExp (TEArrayTypeParam vname) m =
  localVNames
    ( \vnames ->
        vnames
          { vnameEnvAtomTypeVars = S.insert vname $ vnameEnvAtomTypeVars vnames
...
-- | Bind a source type parameter into a local environment.
withISpaceParam :: (MonadCheck m) => ISpaceParam VName -> (ISpaceParam VName -> m a) -> m a
withISpaceParam p m =
  localVNames
    ( \vnames ->
        case p of
...
withType checkTypeExp (p, t) m =
  withTypeParam p $ \p' -> do
    ...
    let f bs =
          bs
            { bindEnvAtomTypes =
                M.insert (unTypeParam p') t' $ bindEnvAtomTypes bs
            }
    localBinds f $ m (p', t')
